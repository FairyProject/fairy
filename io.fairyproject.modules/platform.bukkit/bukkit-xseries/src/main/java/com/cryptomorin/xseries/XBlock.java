...
public final class XBlock {
    ...
    public static boolean isType(Block block, XMaterial material) {
        // LeeGod start - lol fuck you XSeries
        if (!ISFLAT) {
            LegacyMaterial legacy = LegacyMaterial.getMaterial(material.parseMaterial().name());
            if (legacy != null) {
                switch (legacy.handling) {
                    case COLORABLE:
                        byte data = block.getData();
                        if (data == material.getData()) return true;
                        break;
                    case WOOD_SPECIES:
                        throw new UnsupportedOperationException("Wood species is not supported in 1.13 and below.");
                }
            }
        }
        // LeeGod end
